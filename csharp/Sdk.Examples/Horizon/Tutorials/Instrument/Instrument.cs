...
namespace Sdk.Examples.Horizon.Tutorials.Instrument
{
    ...
    public class Instrument : TutorialBase
    {
        ...
        public async Task Search_OpenFigi_Market_Sector()
        {
            ...
            var allowedMarketSectors = await InstrumentApi.GetOpenFigiParameterOptionAsync(OpenFigiParameterOptionName.MarketSector);
            Assert.That(allowedMarketSectors, Contains.Item(new AllowedParameterValue("Equity")));

            // Query OpenFigi with Market Sector filter
            var openFigiMarketSectorResult = await InstrumentApi.SearchOpenFigiAsync("VOD", false, 25, "Equity");
        ...
        public async Task Search_OpenFigi_PermId()
        {
            ...
            var openFigiPermIdResult = await InstrumentApi.SearchOpenFigiAsync("VOD", true);
        ...
        public async Task Vendors(){
            ...
            var allowedSecurityType = await InstrumentApi.GetOpenFigiParameterOptionAsync(OpenFigiParameterOptionName.SecurityType);
