...
public class Valuation {
    ...
    public void run_aggregation_with_buy() throws ApiException {
        run_aggregation(
            () -> {
                    ...
                    requests.add(testDataUtilities.buildCashFundsInTransactionRequest(
                                    new BigDecimal(30600.0), currency, EFFECTIVE_DATE));

                    // create the transaction requests
                    requests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(0),
                                    new BigDecimal(100.0), new BigDecimal(101.0), currency,
                                    EFFECTIVE_DATE, "Buy"));
                    requests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(1),
                                    new BigDecimal(100.0), new BigDecimal(102.0), currency,
                                    EFFECTIVE_DATE, "Buy"));
                    requests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(2),
                                    new BigDecimal(100.0), new BigDecimal(103.0), currency,
                                    EFFECTIVE_DATE, "Buy"));
    ...
    public void run_aggregation_on_multicurrency_portfolio_with_stock_in() throws ApiException {
        ...
        CreateTransactionPortfolioRequest transactionPortfolioRequest = createTransactionPortfolioRequest(
                        portfolioId, portfolioDisplayName, "GBP");

        // prepare transactions across GBP and USD, transactions are transfer in of cash
        // and stock
        List<TransactionRequest> transactionRequests = new ArrayList<>();
        transactionRequests.add(testDataUtilities.buildCashFundsInTransactionRequest(new BigDecimal(10000.0),
                        "GBP", EFFECTIVE_DATE));
        transactionRequests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(0),
                        new BigDecimal(100.0), new BigDecimal(100.0), "USD", EFFECTIVE_DATE, "StockIn"));
        transactionRequests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(1),
                        new BigDecimal(100.0), new BigDecimal(100.0), "GBP", EFFECTIVE_DATE, "StockIn"));
        transactionRequests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(2),
                        new BigDecimal(100.0), new BigDecimal(100.0), "GBP", EFFECTIVE_DATE, "StockIn"));
        ...
        quotes.addAll(asList(
                        ...
                        createMarketDataUpsertQuoteRequest(instrumentIds.get(2), "GBP",
        ...
        List<Map<String, Object>> aggregationResultsByInstrument = runAggregation(portfolioId, quotesScope,
                        GROUPBY_INSTRUMENT_NAME_KEY);
        List<Map<String, Object>> aggregationResultsByPortfolio = runAggregation(portfolioId, quotesScope,
                        GROUPBY_PORTFOLIO_NAME_KEY);
        ...
        assertEquals("ASTRAZENECA PLC should have increased in value and been converted to GBP for the aggregation",
                        13600.0, (double) aggregationResultsByInstrument.get(0).get(AGGREGATION_RESULT_KEY),
                        delta);
        ...
        // verify instruments add up to the portfolio level aggregation in the base
        // currency
        double totalInstrumentValueInBaseCcy = aggregationResultsByInstrument.stream()
                        ...
                        .sum();
        assertEquals(totalInstrumentValueInBaseCcy,
                        aggregationResultsByPortfolio.get(0).get(AGGREGATION_RESULT_KEY));
    }

    private void run_aggregation(
                    Supplier<List<TransactionRequest>> createTransactionRequests,
                    Consumer<List<Map<String, Object>>> validateResults) throws ApiException {
        String uuid = UUID.randomUUID().toString();
        String quotesScope = uuid;
        ...
        // create the aggregation request, this example calculates the percentage of
        // total portfolio value and value by instrument
        ValuationRequest valuationRequest = new ValuationRequest()
                        .recipeId(new ResourceId().scope(recipeScope).code(recipeCode))
                        .metrics(asList(
                                        new AggregateSpec().key(GROUPBY_INSTRUMENT_NAME_KEY)
                                                        .op(AggregateSpec.OpEnum.VALUE),
                                        new AggregateSpec().key(AGGREGATION_KEY)
                                                        .op(AggregateSpec.OpEnum.PROPORTION),
        ...
        /*
        * The aggregation response contains a schema property which describes the data
        * returned.
        ...
        */
        ...
        for (Map<String, Object> aggregation : aggregationResponse.getData()) {
            System.out.println(aggregation.get(GROUPBY_INSTRUMENT_NAME_KEY) + ":\t"
                            + aggregation.get(AGGREGATION_RESULT_KEY) + "\t");
        }
    }

    /**
     * Run an aggregation on LUSID for a given portfolio and scope to produce values
     * in the portfolio
     ...
     * @throws ApiException
     */
    private List<Map<String, Object>> runAggregation(String portfolioId, String quotesScope, String groupingKey)
                    throws ApiException {

        String recipeScope = "tutorials";
        String recipeCode = "multi-currency";

        // create the recipe that will instruct the aggregation on the market data
        // provider and scope.
        ConfigurationRecipe configurationRecipe = new ConfigurationRecipe()
                        .scope(recipeScope)
                        .code(recipeCode)
                        .market(new MarketContext()
                                        // equity and fx data both sourced from Lusid provider, reference the
                                        // createUpsertQuote... methods
     ...
     * @throws ApiException
     */
    private CreateTransactionPortfolioRequest createTransactionPortfolioRequest(String portfolioId,
                    String displayName, String currency) throws ApiException {
            // build the create portfolio request
            CreateTransactionPortfolioRequest request = new CreateTransactionPortfolioRequest()
     ...
     * @return
     */
    private UpsertQuoteRequest createMarketDataUpsertQuoteRequest(String instrument, String currency,
                    BigDecimal value) {
    ...
    private UpsertQuoteRequest createFxUpsertQuoteRequest(String baseCcy, String foreignCcy, BigDecimal rate) {
        return new UpsertQuoteRequest()
                        .quoteId(new QuoteId()
                                        .quoteSeriesId(new QuoteSeriesId()
                                                        .provider("Lusid")
                                                        // base and foreign currencies must be split by "/"
                                                        // (e.g. USD/GBP).
    ...
    private void logResults(String resultSetName, List<Map<String, Object>> results) {
        ...
        for (Map<String, Object> aggregation : results) {
                System.out.println(aggregation.get(resultSetName) + ":\t"
                                + aggregation.get(AGGREGATION_RESULT_KEY) + "\t" +
