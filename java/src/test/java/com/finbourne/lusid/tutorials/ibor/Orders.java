...
public class Orders
{
    ...
    private static Collection<String> TutorialPropertyCodes = Arrays.asList("TIF", "OrderBook", "PortfolioManager",
                    "Account", "Strategy");
    ...
    private static void LoadProperties(PropertyDefinitionsApi propertyDefinitionsApi, Collection<String> scopes,
                    Collection<String> codes) throws ApiException {
    ...
    // We want to make a request for a single order. The LUID will be mapped on
    // upsert
    ...
    public void Upsert_Simple_Order() throws ApiException {
        ...
        // These instrument identifiers should all map to the same instrument. If the
        // instance of
        // LUSID has the specified instruments registered these identifiers will get
        // resolved to
        // an actual internal instrument on upsert; otherwise, they'll be resolved to
        // instrument or
        // currency unknown.
        HashMap<String, String> instruments = new HashMap<>();
        instruments.put("Instrument/default/LusidInstrumentId", instrumentIds.get(0));

        // [Experimental] Currently this portfolio doesn't need to exist. As the domain
        // model evolves
        // this reference might disappear, or might become a strict reference to an
        // existing portfolio.
        ...
        properties.put("Order/" + testScope + "/TIF", new PerpetualProperty().key("Order/" + testScope + "/TIF")
                        .value(new PropertyValue().labelValue("GTC")));
        properties.put("Order/" + testScope + "/OrderBook",
                        new PerpetualProperty().key("Order/" + testScope + "/OrderBook")
                                        .value(new PropertyValue().labelValue("UK Test Orders")));
        properties.put("Order/" + testScope + "/PortfolioManager",
                        new PerpetualProperty().key("Order/" + testScope + "/PortfolioManager")
                                        .value(new PropertyValue().labelValue("F Bar")));
        properties.put("Order/" + testScope + "/Account",
                        new PerpetualProperty().key("Order/" + testScope + "/Account")
                                        .value(new PropertyValue().labelValue("J Wilson")));
        properties.put("Order/" + testScope + "/Strategy",
                        new PerpetualProperty().key("Order/" + testScope + "/Strategy")
                                        .value(new PropertyValue().labelValue("RiskArb")));
        ...
        assertTrue(upsertResult.stream()
                        .allMatch(order -> order.getLusidInstrumentId().equals(instrumentIds.get(0))));
    }

    // We want to make a request for a single order. The LUID will be mapped to an
    // "unknown" identifier on upsert as we're passing unregistered instrument
    // identifiers.
    @Test
    public void Upsert_Simple_Order_With_Unknown_Instrument() throws ApiException {
        ...
        assertEquals(1, upsertResult.stream().count());
        assertTrue(upsertResult.stream()
                        .allMatch(order -> order.getLusidInstrumentId().equals("LUID_ZZZZZZZZ")));
    }

    // We want to make a request for a single order. The internal security id will
    // be mapped on upsert
    // from the instrument identifiers passed.
    @Test
    public void Update_Simple_Order() throws ApiException {
        ...
        assertTrue(upsertResult.stream()
                        .allMatch(order -> order.getLusidInstrumentId().equals(instrumentIds.get(0))));
        assertTrue(upsertResult.stream()
                        .allMatch(order -> order.getQuantity().equals(BigDecimal.valueOf(100.0))));
        ...
        // The return gives us a list of orders upserted, and the lusidinstrument for
        // each has been mapped from the
        // instrument identifiers passed. We can see that the quantity has been udpated,
        // and properties added
        assertEquals(1, updateResult.stream().count());
        assertTrue(updateResult.stream()
                        .allMatch(order -> order.getQuantity().equals(BigDecimal.valueOf(500.0))));
    }

    // We want to make a request for a single order. The internal security id will
    // be mapped on upsert
    // from the instrument identifiers passed. We can filter on a number of
    // parameters on query.
    @Ignore
    @Test
    public void Upsert_And_Retrieve_Simple_Orders() throws ApiException {
        ...
        properties.put("Order/" + testScope + "/TIF", new PerpetualProperty().key("Order/" + testScope + "/TIF")
                        .value(new PropertyValue().labelValue("GTC")));
        properties.put("Order/" + testScope + "/OrderGroup",
                        new PerpetualProperty().key("Order/" + testScope + "/OrderGroup")
                                        .value(new PropertyValue().labelValue("UK Test Orders")));
        properties.put("Order/" + testScope + "/PortfolioManager",
                        new PerpetualProperty().key("Order/" + testScope + "/PortfolioManager")
                                        .value(new PropertyValue().labelValue("F Bar")));
        ...
        altProperties.put("Order/" + testScope + "/OrderGroup",
                        new PerpetualProperty().key("Order/" + testScope + "/OrderGroup")
                                        .value(new PropertyValue().labelValue("UK Test Orders 2")));
        ...
        assertTrue(quantityFilter.stream()
                        .allMatch(order -> order.getQuantity().compareTo(BigDecimal.valueOf(100)) > 0));

        /*
            * List<Order> orderGroupFilter = ordersApi.listOrders(t, null, null, null,
            * null,"Properties[" + testScope + "/OrderGroup] eq 'UK Test Orders 2'",
            * null).getValues();
            * assertEquals(1, orderGroupFilter.size());
            * assertTrue(orderGroupFilter.stream().allMatch(order ->
            * order.getProperties().get("Order/" + testScope +
            * "/OrderGroup").getValue().getLabelValue().equals("UK Test Orders 2")));
            * 
            * List<Order> instrumentFilter = ordersApi.listOrders(t, null, null, null,
            * null,"LusidInstrumentId eq '" + instrumentIds.get(0) + "' and Scope eq '" +
            * testScope + "'", null).getValues();
            * assertEquals(2, instrumentFilter.size());
            * assertTrue(instrumentFilter.stream().allMatch(order ->
            * order.getLusidInstrumentId().equals(instrumentIds.get(0))));
            * 
            * List<Order> sideFilter = ordersApi.listOrders(t, null, null, null,
            * null,"Side eq 'Sell' and Scope eq '" + testScope + "'", null).getValues();
            * assertEquals(1, sideFilter.size());
            * assertTrue(sideFilter.stream().allMatch(order ->
