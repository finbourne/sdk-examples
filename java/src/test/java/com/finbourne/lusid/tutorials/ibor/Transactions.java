...
public class Transactions {
	...
	public void load_listed_instrument_transaction() throws ApiException {
		...
		transactionPortfoliosApi.upsertTransactions(TutorialScope, portfolioId,
						new ArrayList<>(Collections.singletonList(transaction))).execute();
	...
	public void load_cash_transaction() throws ApiException {
		...
		TransactionRequest transaction = new TransactionRequest()
						...
						// Cash instruments are identified using CCY_ followed by the ISO currency
						// codes.
						// Cash instruments do not need to be created before use
						.instrumentIdentifiers(new HashMap<String, String>() {
								{
										put(LUSID_CASH_IDENTIFIER, "GBP");
								}
						})
	...
	public void cancel_transactions() throws ApiException {
		...
		TransactionRequest tx1 = new TransactionRequest()
						...
						.totalConsideration(
										new CurrencyAndAmount().currency("GBP").amount(new BigDecimal(101.0)))
						.transactionDate(effectiveDate.toString())
						.settlementDate(effectiveDate.toString())
						.units(new BigDecimal(100.0));
		TransactionRequest tx2 = new TransactionRequest()
						...
						.totalConsideration(
										new CurrencyAndAmount().currency("GBP").amount(new BigDecimal(102.0)))
						.transactionDate(effectiveDate.toString())
						.settlementDate(effectiveDate.toString())
						.units(new BigDecimal(100.0));
		TransactionRequest tx3 = new TransactionRequest()
						...
						.totalConsideration(
										new CurrencyAndAmount().currency("GBP").amount(new BigDecimal(103.0)))
						.transactionDate(effectiveDate.toString())
						.settlementDate(effectiveDate.toString())
						.units(new BigDecimal(100.0));

		transactionPortfoliosApi.upsertTransactions(TutorialScope, portfolioId, Arrays.asList(tx1, tx2, tx3)).execute();
		...
		List<String> txIdsToDelete = txsBeforeDeletion.stream().map(Transaction::getTransactionId)
						.collect(Collectors.toList());
		transactionPortfoliosApi.cancelTransactions(TutorialScope, portfolioId, txIdsToDelete).execute();
