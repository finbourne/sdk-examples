...
public class Holdings {
    ...
    public void get_holdings() throws ApiException {
        ...
        // add starting cash
        requests.add(testDataUtilities.buildCashFundsInTransactionRequest(new BigDecimal(100000.0), currency,
                        day1));

        // add initial transactions
        requests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(0), new BigDecimal(100.0),
                        new BigDecimal(101.0), currency, day1, "Buy"));
        requests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(1), new BigDecimal(100.0),
                        new BigDecimal(102.0), currency, day1, "Buy"));
        requests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(2), new BigDecimal(100.0),
                        new BigDecimal(103.0), currency, day1, "Buy"));

        // on T+5, add a transaction in instrument 3 and increasing the amount of
        // instrument 1
        requests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(1), new BigDecimal(100.0),
                        new BigDecimal(104.0), currency, dayTPlus5, "Buy"));
        requests.add(testDataUtilities.buildTransactionRequest(instrumentIds.get(3), new BigDecimal(100.0),
                        new BigDecimal(105.0), currency, dayTPlus5, "Buy"));

        // upload the transactions to LUSID
        transactionPortfoliosApi.upsertTransactions(TutorialScope, portfolioCode, requests).execute();
        ...
        assertThat(holdings.getValues().get(2).getUnits(), comparesEqualTo(new BigDecimal(200.0))); // 2
                                                                                                    // transactions
    ...
    public void set_target_holdings() throws ApiException {
        ...
        holdingAdjustments.add(
                        new AdjustHoldingRequest()
                                        .instrumentIdentifiers(new HashMap<String, String>() {
                                                {
                                                        put(LUSID_INSTRUMENT_IDENTIFIER, instrument1);
                                                }
                                        })
                                        .taxLots(Arrays.asList(
                                                        new TargetTaxLotRequest()
        ...
        holdingAdjustments.add(
                        new AdjustHoldingRequest()
                                        .instrumentIdentifiers(new HashMap<String, String>() {
                                                {
                                                        put(LUSID_INSTRUMENT_IDENTIFIER, instrument2);
                                                }
                                        })
                                        .taxLots(Arrays.asList(
                                                        new TargetTaxLotRequest()
        ...
        transactionPortfoliosApi.setHoldings(TutorialScope, portfolioCode, day1.toString(), holdingAdjustments).execute();

        // add subsequent transactions on day 2
        List<TransactionRequest> requests = Arrays.asList(
                        testDataUtilities.buildTransactionRequest(instrument1, new BigDecimal(100.0),
                                        new BigDecimal(104.0), currency, day2, "Buy"),
                        testDataUtilities.buildTransactionRequest(instrument3, new BigDecimal(100.0),
                                        new BigDecimal(103.0), currency, day2, "Buy"));
        transactionPortfoliosApi.upsertTransactions(TutorialScope, portfolioCode, requests).execute();
        ...
        // remaining cash balance which takes into account the purchase transactions on
        // day 2
